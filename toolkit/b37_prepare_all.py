...
def build(x):
    ...
    _sum = r2(sum(D(l['amount']) for l in out))
    _delta = r2(D(rec['subtotal']) - _sum)
    # Ratified check-1 per-line rounding tolerance (v41, Open Item #108): the vendor prints
    # line amounts rounded to 2dp but sums the unrounded values. Never wider than 1c.
    assert abs(_delta) <= D('0.01'), (rec['id'], _sum, rec['subtotal'])
    rec['line_sum'] = f'{_sum:f}'
    rec['check1_delta'] = f'{_delta:f}'
...
json.dump(built, open('/home/claude/built_b37_all.json', 'w'), indent=1)
